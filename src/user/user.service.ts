...
export class UserService {
  ...
  async registration(createUserDto: CreateUserDto): Promise<UserEntity> {
    const IsEmailAlreadyExist: UserEntity = await this.userRepository.findOneBy({ email: createUserDto.email })
    const IsUsernameAlreadyExist: UserEntity = await this.userRepository.findOneBy({ username: createUserDto.username })
    ...
    let createdUser: UserEntity = await this.userRepository.save(newUser)
    delete createdUser.password
    return createdUser
  }

  async login(loginUserDto: LoginUserDto): Promise<IUserResponse> {
    const user: UserEntity = await this.userRepository.findOne({ where: { email: loginUserDto.email }, select: [ 'id', 'username', 'email', 'image', 'bio', 'password' ] })
    if (!user)
      throw new HttpException('User not found', HttpStatus.UNPROCESSABLE_ENTITY)

    const IsCorrectPassword: boolean = await compare(loginUserDto.password, user.password)
  ...
  async update(currentUser: UserEntity, newUser: UserEntity): Promise<UserEntity> {
    const oldUser: UserEntity = await this.getUserById(currentUser.id)
    const updUser: UserEntity = Object.assign(oldUser, newUser)
  ...
  getUserById(uid: number): Promise<UserEntity> {
